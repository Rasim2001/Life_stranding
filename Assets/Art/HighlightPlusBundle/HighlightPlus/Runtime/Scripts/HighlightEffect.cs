...
namespace HighlightPlus
{
    ...
    public partial class HighlightEffect : MonoBehaviour
    {
        ...
        [Tooltip(
            "Optional custom texture property name used for alpha clipping; if not found, falls back to _BaseMap or _MainTex.")]
        ...
        [Tooltip(
            "If enabled, the highlight effect will use a pass from the original object's shader to render the silhouette.")]
        ...
        [Tooltip(
            "Increases the screen coverage for the outline/glow to avoid cuts when using cloth or vertex shader that transform mesh vertices")]
        ...
        [Tooltip("Scale of the overlay pattern")] [Range(1f, 100f)]
        public float overlayPatternScale = 10f;
        [Tooltip("Size/Thickness of the overlay pattern")] [Range(0.01f, 1f)]
        public float overlayPatternSize = 0.15f;
        [Tooltip("Softness of the overlay pattern")] [Range(0.01f, 0.5f)]
        public float overlayPatternSoftness = 0.02f;
        [Tooltip("Rotation angle for the overlay pattern in degrees")] [Range(-180f, 180f)]
        ...
        [Tooltip(
            "Controls how quickly the outline effect scales down with distance when constant width is disabled. Lower values make the effect fade faster with distance.")]
        ...
        [Tooltip("Adds a empty margin between the outline mesh and the effects")] [Range(0, 1)]
        ...
        [Tooltip("Threshold for the pattern texture.")] [Range(0, 1)]
        public float outlinePatternThreshold = 0.3f;
        [Tooltip("Distortion amount for the pattern texture.")] [Range(0, 1.5f)]
        ...
        [Tooltip("Width of the dashed outline.")] [Range(0, 1)]
        public float outlineDashWidth = 0.5f;
        [Tooltip("Gap of the dashed outline.")] [Range(0, 1)]
        ...
        [Tooltip(
            "Controls how quickly the glow effect scales down with distance when constant width is disabled. Lower values make the effect fade faster with distance.")]
        ...
        [Tooltip("Width of the frame when using Frame style")] [Range(0.001f, 0.5f)]
        public float targetFXFrameWidth = 0.1f;
        [Tooltip("Length of the frame corners when using Frame style")] [Range(0, 0.5f)]
        public float targetFXCornerLength = 0.25f;
        [Tooltip("Minimum opacity of the frame when using Frame style")] [Range(0, 1)]
        public float targetFXFrameMinOpacity;
        [Tooltip(
            "If the ground is transparent, the effect won't work. You can set this property to the altitude of the transparent ground to force the effect to render at this altitude.")]
        ...
        [NonSerialized] public bool alignToGroundTried, alignToGroundHitGood;
        ...
        /// </summary>
        [NonSerialized] public float iconFXStartTime;
        ...
        void BuildCommandBuffer(Camera cam, bool clearStencil)
        {
            ...
            if (!ignoreObjectVisibility)
            {
                for (int k = 0; k < rmsCount; k++)
                {
                    if (rms[k].renderer == null || (rms[k].renderer != null &&
                                                    rms[k].renderer.isVisible != rms[k].renderWasVisibleDuringSetup))
                    {
            ...
            if (!needsSetupMaterial)
            {
                ...
                else if ((outline > 0 && outlineQuality == QualityLevel.Highest) ||
                         (glow > 0 && glowQuality == QualityLevel.Highest) && rms[0].fxMatSolidColor == null)
                {
                    needsSetupMaterial = true;
                }
                ...
                else if (seeThrough != SeeThroughMode.Never && seeThroughBorder > 0 &&
                         rms[0].fxMatSeeThroughBorder == null)
                {
                    needsSetupMaterial = true;
                }
            }
                            ...
                            if (outlineQuality.UsesMultipleOffsets())
                            {
                                ...
                                if (allowGPUInstancing)
                                {
                                    ...
                                    if (instanceCount > 0)
                                    {
                                        ...
                                        cbHighlight.DrawMeshInstanced(mesh, l, rms[k].fxMatOutline[l], 1, matrices,
                        ...
                        for (int l = 0; l < subMeshCount; l++)
                        {
                            ...
                            if (usesOverlay)
                            {
                                ...
                                fxMat.SetVector(ShaderParams.OverlayData,
                                    new Vector4(animSpeed, overlayMinIntensity, overlayBlending, overlayTextureScale));
                            ...
                            if (usesInnerGlow)
                            {
                                ...
                                if (rms[k].isCombined)
                                {
                                    cbHighlight.DrawMesh(rms[k].mesh, rms[k].renderingMatrix, rms[k].fxMatInnerGlow[l],
                                        l);
                                }
                                else
                                {
                        ...
                        if (usesGlowNonHQ)
                        {
                                ...
                                if (glowQuality.UsesMultipleOffsets())
                                {
                                    for (int o = glowOffsetsMin; o <= glowOffsetsMax; o++)
                                    {
                                        ...
                                        if (allowGPUInstancing)
                                        {
                                            ...
                                            matDataColor.Add(new Vector4(dataColor.r, dataColor.g, dataColor.b,
                                                dataColor.a));
                                        }
                                        else
                                        {
                                            ...
                                            if (rms[k].isCombined)
                                            {
                                                cbHighlight.DrawMesh(mesh, rms[k].renderingMatrix, rms[k].fxMatGlow[l],
                                                    l);
                                            }
                                            else
                                            {
                            ...
                            if (allowGPUInstancing)
                            {
                                ...
                                if (instanceCount > 0)
                                {
                                    ...
                                    cbHighlight.DrawMeshInstanced(mesh, l, rms[k].fxMatGlow[l], 0, matrices,
                                        instanceCount, glowPropertyBlock);
                                }
                            }
                        }
                            ...
                            if (outlineQuality.UsesMultipleOffsets())
                            {
                                ...
                                for (int o = outlineOffsetsMin; o <= outlineOffsetsMax; o++)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (rms[k].isCombined)
                                        {
                                            cbHighlight.DrawMesh(mesh, rms[k].renderingMatrix, rms[k].fxMatOutline[l],
                                                l, 0);
                                        }
                                        else
                                        {
                ...
                if (targetFX)
                {
                    ...
                    if (fadeOut > 0 && !(targetEffectRendered && (usesTarget || targetFXUseEnclosingBounds)))
                    {
                        ...
                        if (targetFXAlignToGround)
                        {
                            ...
                            alignToGroundHitGood = Physics.Raycast(center, Vector3.down, out RaycastHit groundHitInfo,
                        ...
                        else
                        {
                            ...
                            if (targetFxAlignToNormal && currentHitNormal != Vector3.zero)
                            {
                                ...
                                rotation *= Quaternion.Euler(0, 0,
                                    targetFXRotationAngle + time * targetFXRotationSpeed);
                            }
                            else
                            {
                    ...
                    if (fadeOut > 0 && !(iconEffectRendered && usesTarget))
                    {
                        ...
                        switch (iconFXAnimationOption)
                        {
                            case IconAnimationOption.VerticalBounce:
                                center.y += iconFXAnimationAmount *
                                            Mathf.Abs(Mathf.Sin((now - iconFXStartTime) * iconFXAnimationSpeed));
                                break;
                        }
                    ...
                    for (int l = 0; l < subMeshCount; l++)
                    {
                        ...
                        else
                        {
                            if (l < rms[k].fxMatSolidColor.Length)
                            {
                                if (rms[k].isCombined)
                                {
                                    cbHighlight.DrawMesh(rms[k].mesh, rms[k].renderingMatrix, rms[k].fxMatSolidColor[l],
                                        l);
                                }
                                else
                                {
        ...
        bool ComputeSmoothQuadMatrixOriginShifted(Camera cam, ref Bounds bounds, ref Vector3 shift)
        {
            ...
            if (outline > 0)
            {
                BuildMatrix(cam, scrMin, scrMax, (int)(10 + 20 * realOutlineWidth + 5 * outlineDownsampling),
        ...
        void SmoothGlow(int rtWidth, int rtHeight)
        {
            ...
            else
            {
                ...
                for (int k = 0; k < bufferCount - 1; k++)
                {
                    ...
                    RenderingUtils.FullScreenBlit(cbHighlight, mipGlowBuffers[k], mipGlowBuffers[k + 1], fxMatBlurGlow,
                        3);
                }
            }
        }
        ...
        void SmoothOutline(int rtWidth, int rtHeight)
        {
            ...
            if (!constantWidth)
            {
                widthScale = Mathf.Max(minimumWidth,
                    1f - Mathf.Log(1 + smoothEffectDistance / outlineDistanceScaleBias));
        ...
        void SetupMaterial(Renderer[] rr)
        {
            ...
            for (int k = 0; k < rr.Length; k++)
            {
                ...
                {
                    if (renderer.TryGetComponent(out HighlightEffect otherEffect) && otherEffect.enabled &&
                        otherEffect.ignore)
                    {
        ...
        void ConfigureOutput()
        {
            ...
            cbHighlight.SetRenderTarget(colorAttachmentBuffer, RenderBufferLoadAction.Load,
                RenderBufferStoreAction.Store, depthAttachmentBuffer, RenderBufferLoadAction.Load,
 ...
 colorAttachmentBuffer != BuiltinRenderTextureType.CameraTarget && depthAttachmentBuffer == BuiltinRenderTextureType.CameraTarget;
        ...
        public void UpdateMaterialProperties(bool disabling = false)
        {
            ...
            bool outlineNeedsStencil = outlineWantsMask &&
                                       (outlineQuality != QualityLevel.Highest ||
                                        a_outlineMaskMode != MaskMode.CutoutOnly);
            bool glowNeedsStencil = glowWantsMask &&
                                    (glowQuality != QualityLevel.Highest || a_glowMaskMode != MaskMode.CutoutOnly);
            ...
            renderMaskOnTop = _highlighted && ((outline > 0 && outlineVisibility != Visibility.Normal) ||
                                               (glow > 0 && glowVisibility != Visibility.Normal) ||
            ...
            if (useSmoothGlow)
            {
                ...
                fxMatComposeGlow.SetInt(ShaderParams.GlowStencilComp,
                    (a_glowMaskMode == MaskMode.IgnoreMask || a_glowMaskMode == MaskMode.CutoutOnly)
            ...
            if (useSmoothOutline)
            {
                ...
                if (outlineStylized)
                {
                    ...
                    fxMatComposeOutline.SetVector(ShaderParams.PatternData,
                        new Vector4(outlinePatternScale, outlinePatternThreshold,
                            outlinePatternDistortionAmount * 0.01f, outlinePatternStopMotionScale));
                }
                else
                {
                ...
                if (outlineDashed)
                {
                    ...
                    fxMatComposeOutline.SetVector(ShaderParams.DashData,
                        new Vector4(1.001f - outlineDashWidth, 1f - outlineDashGap * 2f, outlineDashSpeed, 0));
                }
                else
                {
                ...
                if (outlineEdgeMode != OutlineEdgeMode.Exterior || a_outlineMaskMode == MaskMode.IgnoreMask ||
                    a_outlineMaskMode == MaskMode.CutoutOnly ||
                    (outlineQuality == QualityLevel.Highest && outlineStylized))
                {
                ...
                if (outlineColorStyle == ColorStyle.Gradient)
                {
                    ...
                    fxMatComposeOutline.SetVector(ShaderParams.OutlineGradientData,
                        new Vector4(outlineGradientKnee, outlineGradientPower, 0, 0));
            ...
            if (targetFX)
            {
                ...
                fxMatTarget.SetVector(ShaderParams.TargetFXFrameData,
                    new Vector4(targetFXFrameWidth, targetFXCornerLength, targetFXFrameMinOpacity,
                        targetFXGroundMinAltitude));
                    ...
                    for (int l = 0; l < subMeshCount; l++)
                    {
                        ...
                        if (rms[k].fxMatMask != null && rms[k].fxMatMask.Length > l)
                        {
                            ...
                            if (fxMat != null)
                            {
                                ...
                                fxMat.SetFloat(ShaderParams.Padding,
                                    outlineQuality != QualityLevel.Highest ? padding * 0.01f : 0);
                                if (renderMaskOnTop)
                                {
                        ...
                        if (rms[k].fxMatGlow != null && rms[k].fxMatGlow.Length > l)
                        {
                            ...
                            if (fxMat != null)
                            {
                                ...
                                fxMat.SetVector(ShaderParams.Glow2,
                                    new Vector4(outline > 0 ? realOutlineWidth / 100f : 0, animSpeed, glowDithering));
                        ...
                        if (rms[k].fxMatOverlay != null && rms[k].fxMatOverlay.Length > l)
                        {
                            ...
                            if (fxMat != null)
                            {
                                ...
                                fxMat.SetVector(ShaderParams.OverlayPatternData,
                                    new Vector4(overlayPatternScale, overlayPatternSize, overlayPatternSoftness,
                                        overlayPatternRotation));
                        ...
                        if (rms[k].fxMatInnerGlow != null && rms[k].fxMatInnerGlow.Length > l)
                        {
                            ...
                            if (fxMat != null)
                            {
                                ...
                                fxMat.mainTextureOffset = matTextureOffset;
                                fxMat.mainTextureScale = matTextureScale;
                                fxMat.SetVector(ShaderParams.InnerGlowData,
                                    new Vector4(innerGlowWidth, innerGlowPower, 0, 0));
        ...
        static readonly Dictionary<Mesh, HashSet<HighlightEffect>> sharedMeshUsage =
            new Dictionary<Mesh, HashSet<HighlightEffect>>();
        ...
        public string ValidateCombineMeshes()
        {
            ...
            {
                if (!rm.isSkinnedMesh && rm.mesh != null && !rm.mesh.isReadable)
                    return
                        $"The mesh of object {rm.renderer.name} is not readable and can't be combined (check mesh's import settings).";
            }
