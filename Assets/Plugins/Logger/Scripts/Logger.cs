...
namespace EasyUI
{
    ...
    public class Logger : MonoBehaviour
    {
        ...
        [Space(50f)] [SerializeField] private GameObject uiEventSystem;
        ...
        private void LogCallback(string message, string stackTrace, LogType type)
        {
            ...
            string formattedStackTrace = !string.IsNullOrEmpty(stackTrace)
                ? $"<color=#cccccc>\nStackTrace: {stackTrace}</color>\n"
                : string.Empty;

            uiLogText.text +=
                $"<sprite={logTypeIndex}><color={colors[logTypeIndex]}> {message}</color>{formattedStackTrace}\n\n";
